/**
 * Client-Side Logic Engine — Deterministic Financial Calculations
 * Runs as seamless fallback whenever local backend server is offline or unreachable.
 */
...
export function calculateFinancialScore(income, expenses, savings) {
  let score = 0;
...
export function calculateSIP(savings) {
  if (savings <= 0) return 0;
  return Math.round(savings * 0.5);
}

export function calculateGoalTimeline(savings, goal, goalAmount) {
...
export function calculateLoanVsSipStrategies(targetAmount, savings, goalCategory = 'house', customConfig = {}) {
...
export function runClientSideAnalysis(data) {
  ...
  const aiAdvice = `### Sovereign Strategic Assessment (${(goal || 'Wealth').toUpperCase()})

#### 1. Financial Health Overview
- **Monthly Income**: ₹${income.toLocaleString('en-IN')}
- **Monthly Expenditure**: ₹${expenses.toLocaleString('en-IN')}
- **Net Monthly Surplus**: ₹${savings.toLocaleString('en-IN')} (${expenseAnalysis.savingsPercent}% savings rate)
- **Financial Health Score**: **${score}/10** (${expenseAnalysis.rating.replace('_', ' ').toUpperCase()})

#### 2. Strategic Goal Analysis (${goalTimeline.targetAmount ? '₹' + goalTimeline.targetAmount.toLocaleString('en-IN') : 'Custom Target'})
- **Option A (Pure Auto/Home Loan)**: EMI of ₹${strategies.onlyLoan.monthlyEmi.toLocaleString('en-IN')}/month over ${strategies.onlyLoan.tenureYears} years @ ${strategies.onlyLoan.interestRate}%. Total interest paid will be ₹${strategies.onlyLoan.totalInterestPaid.toLocaleString('en-IN')}.
- **Option B (Pure SIP Investment - Zero Debt)**: Invest ₹${strategies.onlySip.monthlySip.toLocaleString('en-IN')}/month @ 12% CAGR. Reach your target in ${strategies.onlySip.yearsNeeded} years without paying a single Rupee in interest.
- **Option C (Hybrid Accelerated Path)**: Put down ₹${strategies.hybrid.downPayment.toLocaleString('en-IN')} upfront, keep EMI at ₹${strategies.hybrid.monthlyEmi.toLocaleString('en-IN')}/month, and invest parallel SIP of ₹${strategies.hybrid.parallelSip.toLocaleString('en-IN')}/month to prepay the loan in ~${strategies.hybrid.payoffEstimateYears} years.

#### 3. Sovereign Wealth Recommendation
We strongly recommend allocating at least **₹${sip.toLocaleString('en-IN')}/month** into high-grade equity index mutual funds while keeping 6 months of expenses in an emergency liquid debt fund.`;
  ...
  return {
    ...
    aiAdvice,
    suggestions: expenseAnalysis.suggestions
  };
}
